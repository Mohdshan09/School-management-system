...
export default function Navbar() {
  ...
  const [selectedItem, setSelectedItem] = useState<keyof typeof itemDetails | null>(null);
    ...
    <div className="relative w-full bg-gray-50">
      ...
      <nav className=" w-full bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between fixed top-0 left-0 right-0 z-50">
        ...
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-sm font-bold text-gray-700">BHN HR SEC ENGLISH SCHOOL</h1>
        </div>
        ...
        <div className="flex items-center space-x-6">
          <NavItem icon={<MessageSquare className="w-4 h-4" />} label="My Messages" />
          <NavItem icon={<FilePlus className="w-4 h-4" />} label="New Admission" />
          ...
          <div className="relative">
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-2 w-80 bg-white border border-gray-300 shadow-lg rounded-xl p-4 grid grid-cols-3 gap-2 z-50">
                {Object.entries(itemDetails).map(([label, { icon }]) => (
                  <DropdownItem
                    key={label}
                    icon={icon}
                    label={label}
          ...
          <NavItem icon={<CreditCard className="w-4 h-4" />} label="Collect Fee" />
